...
import { getRandomColor,getShortDescription, getShortTags } from 'src/services/helpers';
import Link from 'next/link';

const Item: NextPage<ItemType> = ({ value }: ItemType) => {
  const getPrice = () => {
    if(value.sellType === 'RANGE') return <>${value.priceMin}&#8377; - ${value.priceMin}&#8377;</>
  ...
  return (
    ...
    <div className='flex flex-col gap-4 h-auto p-4'>
      <div className="flex flex-col h-auto px-6">
        ...
        <p className="mb-2 tooltip tooltip-warning tooltip-down block font-sans text-sm h-3/6 font-normal leading-normal text-gray-700 antialiased opacity-75" data-tip={value.description}>
      ...
      <div className="px-6">
        <button className="btn btn-wide btn-info"><Link href={`/items/${value.id}`}>Buy</Link></button>
      </div>
    </div>
